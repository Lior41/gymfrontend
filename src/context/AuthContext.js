...
export const AuthProvider = ({ children }) => {
  ...
  const login = async (userName, password) => {
    try {
      const response = await axios.post(
        "https://gymbackend-80ub.onrender.com/api/users/signin",
        { userName, password }
      );
  ...
  async function register(userName, password) {
    try {
      const response = await axios.post(
        "https://gymbackend-80ub.onrender.com/api/users/signup",
        { userName, password }
      );
  ...
  async function loadUserFromToken() {
    ...
    if (token) {
      ...
      try {
        const response = await axios.get("https://gymbackend-80ub.onrender.com/api/users/me");
